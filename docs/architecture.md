...
- SSH 远端操作采用逐服务器 TOFU 指纹确认：探测可读取当前指纹，任务、公钥部署和恢复重连均校验已确认 fingerprint；指纹变化必须显式确认更新。密码首次接入保留，密钥验证成功后才允许清除控制器保存的密码。
...
### tasks API (`/api/tasks`)
- 任务创建、批量创建（`/batch`）
- GPU 驱动安装（`/gpu-driver/rocky9`、`/gpu-driver/batch`）：接口路径为兼容保留名称；任务展示为“自动识别系统”，并按目标服务器 OS 自动选择 Rocky 9 或 Ubuntu 安装脚本，支持 GeForce / Data Center（RTX Enterprise）驱动库与临时 `.run` 上传。Rocky 9 请求可额外传入 `allow_kernel_maintenance=true`；默认及字段省略均为 `false`。仅在当前运行内核缺少精确 `kernel-devel`/`kernel-headers`、且操作员明确授权时，任务才会从已锁定的同一 Rocky 小版本仓库安装精确候选内核包，绝不以 `yum update`/`dnf update` 代替。候选内核必须先生成非空 initramfs，并按根文件系统实际依赖校验 NVMe、device-mapper/LVM 与 XFS/ext4 模块，再切换默认启动项和重启；任何候选、镜像或模块校验失败都会恢复/保留旧默认项并终止。重连后任务必须确认 `uname -r` 为候选内核，重新执行发行版锁定脚本锁定已验证内核，才恢复 Nouveau/驱动安装阶段；未授权时只返回维护窗口指引，不修改远端内核或启动项。
- CUDA Toolkit 安装（`/cuda-toolkit`、`/cuda-toolkit/batch`）：支持 11.8、12.0–12.6、12.8、12.9、13.0，安装前校验 `nvidia-smi`，仅安装 Toolkit，不安装或覆盖驱动。Ubuntu 路径在每个 `apt`/`dpkg` 操作前检查锁：人工 `apt`/`dpkg`、cloud-init 或未识别锁只等待，最长 900 秒；仅无下载和 CPU 进展持续约 10 秒的 `apt-daily-upgrade.service` 自动更新会被发送 `SIGTERM`，随后执行 `dpkg --configure -a` 恢复，不删除锁文件或强杀进程。
- CUDA Toolkit 单次失败/取消任务的“重跑”会复用保存的 CUDA 版本、强制安装选项与 OS profile，创建新的 CUDA 专用任务并由 CUDA runner 执行；不会将内部标识 `cuda-toolkit/auto` 当成资产库脚本路径校验。
- 压测套件创建（`/stress-suite`）：同服务器内 GPU → CPU/内存保持串行；选择多个磁盘时，前置阶段结束后各挂载点并行推进
- 极限压测（`extreme_stress_report.sh`）由编排脚本同步调用 GPU 与 CPU/内存子脚本，不调用磁盘脚本；提交前执行只读预检，运行参数保留预检快照，资源域冲突按 GPU / CPU-内存拒绝并审计。
- 受控极限压测可在远端工作目录生成版本化 `diagnosis.jsonl`，每行以 `version:1`、阶段、结果、分类和脱敏消息记录诊断事件；回收后由报告摘要聚合，旧任务继续使用日志回退。
...
- 磁盘压测脚本 `v2026.08.21.2` 支持已挂载路径的 fio 随机混合读写稳定性测试。每次远端启动时会沿块设备父链解析实际底层盘，并以 `ROTA`、`TRAN`、`MODEL` 和 NVMe 设备名自动定档：机械盘为 `2 worker × 1G`，SSD 为 `4 × 2G`，NVMe 为 `8 × 2G`；RAID 控制器逻辑盘（例如 MegaRAID `MRxxxx`）标识为 `RAID`，不把控制器的 `ROTA=1` 误作底层机械盘。SSH 探测同时把每个挂载点的 `physical_device` 返回给任务页；默认选择按该物理盘分组，同盘优先非根目录挂载点，只有根目录时才选择根目录，因此每块盘默认只创建一个磁盘子任务。无法可靠识别的设备保守使用机械盘档。默认 fio 负载明确为 `4K randrw`、读 30%/写 70%、`psync`、`direct=1`、每次写入 `fdatasync=1`，并在负载结束后执行 CRC32C 校验，避免 stress-ng 的隐式读模式和页缓存误判。fio 使用默认的按 job/file 编号生成的独立测试文件，避免并发 job 覆盖；JSON 指标提取兼容 fio 前置提示行，但没有有效 payload 时任务报告明确 FAIL，绝不再将指标静默填为 0。启动前按文件系统总容量的 10% 与 20GiB 中较大值保留安全余量；自动档在必要时先减少 worker、再将单 worker 工作集降至 1G，最低档仍会侵占余量才拒绝启动。用户显式传入 `WORKERS` 时保持手动覆盖语义并使用每 worker 20G，不自动降档。TXT/XLSX 报告记录自动档位、旋转属性、传输链路、安全余量、总工作集、完整 fio I/O 路径及读/写带宽、IOPS、平均/p95/p99 延迟；XLSX 附带 fio JSON 原始结果，文件名保留时间戳并追加 `root` 或挂载目录标识。每台服务器只会为自己默认或手动勾选的挂载点创建 `磁盘压测 · <目录>` 子任务，同服务器磁盘任务共享 GPU/CPU 等前置阶段并行执行，不同服务器批次也并行。批次 ZIP 保留每份 XLSX 的原始文件名并导出每个已选挂载点的报告。
- 并行创建的压测子任务在远端工作目录中追加各自 `task_id`，避免同秒创建时共享脚本、PID、日志或报告文件。
- 磁盘压测 `v2026.09.11.1` 在 fio 单阶段能力之上，先以用户指定时长执行性能主测，再执行持久化校验。性能主测使用 4K `randrw`、读 30%/写 70%、`libaio`、`direct=1`，并按 HDD/SSD/NVMe 使用 4/16/32 队列深度；报告同时给出每 worker 队列深度与总队列深度。持久化校验使用 4K `randwrite`、`psync`、逐写 `fdatasync=1` 与 CRC32C，并取消该阶段的 `time_based/runtime`，确保 fio 在固定工作集写满后进入实际回读校验；工作集按性能主测时长和介质分档：不超过 3 分钟为 8MiB/worker、3–60 分钟为 32MiB/worker；超过 60 分钟时 HDD（含保守 HDD 变体）为 32MiB/worker，SSD/NVMe 与未知介质为 256MiB/worker。任一阶段的 fio JSON 缺失、无效、I/O 错误或校验失败均判 FAIL。进入耐久阶段时脚本先输出 `[STAGE] durability_verify_start`，使任务日志明确显示“耐久校验中”。内核错误监听优先使用 `dmesg --follow-new`；旧版 util-linux 缺少该参数时，使用 `dmesg -w` 并按启动时 `/proc/uptime` 过滤历史 ring buffer，且仅匹配测试挂载点的块设备及其父链；监听器不可用会明确判 FAIL，避免把不可观测状态误记为通过。仅当 CentOS Linux 8 的启用 repo 仍指向失效 mirrorlist 且 `dnf makecache --refresh` 失败时，脚本才备份 BaseOS/AppStream/Extras repo、切换到固定 `8.5.2111` archive 并复验；archive 复验失败自动恢复原 repo。持久化阶段是性能主测后的附加尾段，任务完成时间会略长于用户设定时长；TXT/XLSX 明确分区展示性能与持久化指标。
- 运行中任务的结构化监控接口保留文件系统容量快照，并新增基于 `iostat -dx 1 2` 第二个区间样本的设备级磁盘 I/O 指标：读/写 IOPS、带宽、读/写 await、队列深度与 util；缺少 `iostat` 时明确返回不可用状态。磁盘压测脚本将 `sysstat` 作为依赖，确保新任务可采样。
- 任务执行器会保留预分配的远端目录，仅将 `~/` 展开为远端 HOME；不会在连接后重新生成目录覆盖并行隔离标识。
- CPU/内存压测脚本 `v2026.09.11.3` 保持 `stress-ng` 全 CPU worker 与内存校验方式，CSV、TXT 和 XLSX 报告新增基于 `/proc/stat` 的 CPU Busy、I/O Wait、Steal，以及内存压力目标达成率。CPU Busy 平均低于 80% 或内存目标最高达成率低于 75% 时判定 FAIL，阈值允许通过环境变量覆盖；CPU Busy 平均和最高值默认排除最初 15 秒预热阶段（按采样间隔向上取整），并只统计 `stress-ng` 仍在运行的完整采样窗口，避免短时任务把压力启动爬升或结束后的收尾空闲采样计入判定；CSV 记录 `stress_active`，TXT/XLSX 报告记录实际有效压力采样数。内存目标仍以启动时可用内存的 85% 为上限；安全余量取“物理内存 10%”与“物理内存 25%（最高 4 GiB）”的较大值，目标缓冲取物理内存 2% 并限制在 128–512 MiB，从而兼容小内存测试机和大内存服务器。运行中可用内存连续 3 个采样周期跌破安全余量时终止 `stress-ng` 并判定 FAIL，不做自动减载或重启 VM worker。CPU 温度优先读取 Intel `Package id`、AMD `Tctl/Tdie`，`sensors -j` 不可用时回退 Linux `hwmon`；温度当前只记录、不参与 PASS/FAIL，也尚未进入任务实时监控接口。仅当 CentOS Linux 8 的失效 mirrorlist 使 DNF 刷新失败时，脚本备份并切至固定 `8.5.2111` archive，archive 不可用则恢复原 repo；若该系统在修复 repo 后仍无 `stress-ng` RPM，才以 `min(nproc, 8)`（可由 `HPCDEPLOY_STRESS_NG_BUILD_JOBS` 覆盖）构建固定上游 tag `V0.22.00` 并安装到 `/usr/local/bin`，并只对剩余的非空依赖包重试 DNF。其他发行版与已可用的 CentOS repo 保持原有依赖安装路径。内核监听优先使用 `dmesg --follow-new`，旧版 util-linux 使用按启动 `/proc/uptime` 过滤历史 ring buffer 的 `dmesg -w` 回退，并在结束时按开始时间二次核验 `journalctl -k`；`UE`/`UECC`、可纠正 ECC MCE、OOM、热节流和一般 MCE/硬件事件分别判定与说明。TXT、XLSX 和 Shell 输出均显示实际事件分类；XLSX 的事件明细 Sheet 按分类命名（如 `CorrectedECC`、`UncorrectableECC`、`OutOfMemory`），原始 `cpu_mem_error_*.log` 文件名保持兼容。`openpyxl` 为必需依赖：缺失、无法导入或未生成非空 XLSX 时，脚本明确失败，TXT/CSV-only 不得成功。
- CPU/内存压测脚本 `v2026.09.11.4` 取代上述 `v2026.09.11.3` 的源码编译并发上限：CentOS Linux 8 无 `stress-ng` RPM 时，默认以 `nproc` 返回的当前可调度线程数执行 `make -j`，不再限制为 8；`HPCDEPLOY_STRESS_NG_BUILD_JOBS` 仍可供共享机器显式降低或指定并发。其余 CPU/内存压测、依赖修复、事件监听和报告判定行为保持不变。
...
- Linux 当前版本锁定脚本 v1.7.7 在 Rocky 锁源时显式禁用遗留的 `[extras]` repo，并在 DNF enabled repo 校验中拒绝其仍然启用的状态；避免已停止提供旧小版本 mirrorlist 的 `extras` 元数据阻断后续 NVIDIA 驱动依赖安装。原 `.repo` 文件仍会先完整备份，锁源失败时沿用既有回滚恢复。
...
- 仪表盘近期完成列表新增 `recent_completed_batches`：先按批次最近结束时间选取候选，再加载候选批次全部子任务，复用历史页的 retry / report 最终状态语义汇总成功、失败、取消、服务器数、子任务数和耗时；首页按批次/单次混合行展示，显示数量按聚合行计算。运行中任务继续逐子任务展示。
- Dashboard 不展示控制器空间卡片，首屏聚焦运行状态、运行中任务和近期完成任务。`/api/dashboard/summary` 保留兼容性的 storage 基础字段，但不执行项目目录递归扫描；项目级空间统计由系统设置路径能力或后续独立空间接口提供，避免拖慢仪表盘。
- Dashboard 服务器管理只统计在管服务器的 `online` / `offline`；只有最近 1 小时内探测成功的 `online` 才计入在线，过期、未探测或没有明确状态的在管服务器计入离线。已归档服务器不参与状态统计，只单独展示归档数量。
...
- 结果文件入口先展示 artifact/result 文件列表，再由用户选择具体文件下载。单次文件下载与批次 ZIP 下载均以 Blob 请求接收完整 HTTP 响应、从 `Content-Disposition` 取得文件名后触发浏览器保存；仅在响应成功后关闭结果文件弹窗，用户回到当前历史任务列表。请求失败时弹窗和文件列表保持不变并显示错误，方便直接重试。这里的“已下载”表示前端已收齐文件并已交给浏览器下载管理器，不承诺操作系统本地落盘已完成。
...
- Windows 分类仅接受 `.ps1`、`.bat`、`.cmd`，单文件不超过 2 MiB；只供 Windows 压测页面预览、下载和复制，不可创建 Linux 任务。页面以“常规压测 / 补测命令”双卡生成供人工复制的 PowerShell 命令：常规压测可选整机、GPU、CPU/内存或磁盘，每个模块时长由小时与分钟独立输入并转换为脚本参数；整机模式明确展示三个阶段的合计时长。补测命令默认 GPU，支持 GPU、CPU/内存或磁盘；操作者提供原报告目录后通过 `-MergeBaseReportDir` 合并，以原报告为基线仅替换本次模块数据，并按合并结果重新生成完整 HTML。未补测的 GPU、CPU/内存和磁盘采样及专用原始数据保留，并作为完整报告中的正常模块结果参与状态、阶段时间、指标和趋势图。整体结论、模块执行状态、六、测试项目状态、指标汇总和磁盘阈值均只以合并后的有效采样和 DiskSpd 结果重评；本次补测的“未启用/未启动”标记不得覆盖已保留模块。`-RebuildReportDir` 可将已有融合报告目录的原始数据离线复制到新目录并重建 HTML/ZIP，不准备工具、不启动压测进程。磁盘补测默认按物理盘执行，也可指定 `D:` 等盘符；同一物理盘分区只选一个非系统分区，避免并发 DiskSpd 争用造成不可信的分区级评级。每块候选盘独立校验测试文件大小和额外 5GB 安全余量：空间不足时记录 `[DISK SKIP]` 并从实际测试盘列表移除，不中止其他合格盘及 GPU/CPU 阶段；若全部候选盘不足，则仅跳过磁盘阶段并在报告中说明原因。双卡的模块、时长及命令区按相同行对齐，复制按钮固定在各卡右上角。当前 `v100_windows_stress.ps1` 下载完整 7-Zip 24.09 x64 时仅使用项目内网镜像 `171.221.252.54:8573`，不校验哈希且不回退官方源；镜像不可达时，脚本将记录告警并尝试 PowerShell 内置解压。管理员 PowerShell、PawnIO 未安装且 `AutoConfirmPawIoInstall=true` 时，会从 PawnIO 官方 GitHub Release 下载 Authenticode 有效签名的安装器并以 `-install -silent` 安装；安装前优先检查 PawnIO 卸载注册表项及 `Program Files\PawnIO\PawnIOLib.dll`，避免 LibreHardwareMonitor 状态误判导致重复下载。仅接受退出码 0 或 3010，且会轮询复核安装状态。非管理员、签名校验失败或超时只记录告警并降级采集。报告的核心指标和 CPU/GPU 分项指标均展示 CPU 最高、平均温度，以及 GPU 最高、平均最高温度和最大、平均总功耗；CPU 温度在所有 CPU 类型统一按 ≤95°C 通过、>95°C 且 <100°C 关注、≥100°C 不合格评定。GPU 优先按驱动的 `power.limit`、`GPU Slowdown Temp` 动态评定，热降频点不可用时按现有 GPU 温度阈值评定。CPU 优先使用 LHM PPT/Power Limit 百分比反推平台上限，未读取到时可回退到本地精确型号的官方 TDP；达到基准显示通过，低于基准显示 `-`。客户报告的参考标准面板只展示已成功读取的动态限制值。
- `v100_windows_stress.ps1` 启动 y-cruncher 前以 x64/x86 `MSVCP140.dll`（最低 `14.51.36247.0`）而非“DLL 是否存在”判定 VC++ 2015–2022 Runtime 可用性，避免旧版运行库造成 `0xC0000005` 崩溃被漏检。版本不足时仅管理员 PowerShell 可从微软固定官方入口 `https://aka.ms/vc14/vc_redist.x64.exe` 与 `https://aka.ms/vc14/vc_redist.x86.exe` 下载，验证 Authenticode 的 Microsoft Corporation 签名后以 `/install /quiet /norestart` 修复；只接受 0、1638、3010 后复核 DLL 版本。非管理员、下载/安装失败或复核不足时跳过 y-cruncher，交给既有 CPU/内存 fallback，并写入原因。
- `v100_windows_stress.ps1` 的 y-cruncher CPU/内存阶段不将大 NUMA 主机的线程和内存准备期计入 `CpuMinutes` / `DurationHours`：启动后每 10 秒检查进程、总 CPU 与已用内存，CPU 连续两次至少 80%、内存达到 `YCruncherMemoryPercent - 5` 才开始正式计时。默认 `YCruncherPreparationTimeoutSeconds=900`；未就绪时停止 y-cruncher，并将 CPU 模块明确标记为“准备超时”，而不是把初始化低负载误判为硬件不合格。AMD EPYC `9Vxx` 识别为 Zen 5，优先选择 `24-ZN5 ~ Komari.exe`。
- y-cruncher 的实际后台 PID 而非固定进程名用于就绪判断，兼容 `24-ZN5 ~ Komari.exe` 等专用后端；脚本不再传递会从启动时就倒计时的 `-TL`，达到有效压力后由外层统一执行用户设定时长。准备日志只在内存使用跨越 10% 时输出；正式阶段只记录 0%、25%、50%、75%、100% 进度，进度点严格按任务总时长换算（例如 12 小时为每 3 小时，24 小时为每 6 小时）。
- Windows 分类仅接受 `.ps1`、`.bat`、`.cmd`，单文件不超过 2 MiB；只供 Windows 压测页面预览、下载和复制，不可创建 Linux 任务。页面以“常规压测 / 补测命令”双卡生成供人工复制的 PowerShell 命令：常规压测可选整机、GPU、CPU/内存或磁盘，每个模块时长由小时与分钟独立输入并转换为脚本参数；整机模式明确展示三个阶段的合计时长。补测命令默认 GPU，支持 GPU、CPU/内存或磁盘；操作者提供原报告目录后通过 `-MergeBaseReportDir` 合并，原报告中未补测模块的采样与 `diskspd_*.log` 保留，本次对应模块数据替换原结果。磁盘补测默认按物理盘执行，也可指定 `D:` 等盘符；同一物理盘分区只选一个非系统分区，避免并发 DiskSpd 争用造成不可信的分区级评级。双卡的模块、时长及命令区按相同行对齐，复制按钮固定在各卡右上角。当前 `v99_windows_stress.ps1` 下载完整 7-Zip 24.09 x64 时仅使用项目内网镜像 `171.221.252.54:8573`，不校验哈希且不回退官方源；镜像不可达时，脚本将记录告警并尝试 PowerShell 内置解压。管理员 PowerShell、PawnIO 未安装且 `AutoConfirmPawIoInstall=true` 时，会从 PawnIO 官方 GitHub Release 下载 Authenticode 有效签名的安装器并以 `-install -silent` 安装；安装前优先检查 PawnIO 卸载注册表项及 `Program Files\PawnIO\PawnIOLib.dll`，避免 LibreHardwareMonitor 状态误判导致重复下载。仅接受退出码 0 或 3010，且会轮询复核安装状态。非管理员、签名校验失败或超时只记录告警并降级采集。报告的核心指标和 CPU/GPU 分项指标均展示 CPU 最高、平均温度，以及 GPU 最高、平均最高温度和最大、平均总功耗；CPU 温度在所有 CPU 类型统一按 ≤95°C 通过、>95°C 且 <100°C 关注、≥100°C 不合格评定。GPU 优先按驱动的 `power.limit`、`GPU Slowdown Temp` 动态评定，热降频点不可用时按现有 GPU 温度阈值评定。CPU 优先使用 LHM PPT/Power Limit 百分比反推平台上限，未读取到时可回退到本地精确型号的官方 TDP；达到基准显示通过，低于基准显示 `-`。客户报告的参考标准面板只展示已成功读取的动态限制值。
- 融合磁盘补测会保留原磁盘采样为“原测”阶段，并追加独立“补测”阶段；总体起止时间、GPU/CPU 阶段时长均从合并采样重算，工具信息从原报告事件日志补齐，顺序读写最终指标按每个盘符展示。
...
管理员权限只在服务端成功验证管理员密码后签发；系统不提供临时、开发或环境开关控制的无密码授权路径。
...
若 `/opt/software/gpu-burn/Makefile` 在压测启动前不存在，脚本将其视为受控依赖缺失：恢复前先创建 `/opt/software`（即受控源码与归档缓存的父目录），仅此时使用已有 `/opt/software/gpu-burn-master.zip`，或在该缓存包缺失时从用户指定共享地址下载后持久保留；随后检查归档路径安全、解压并原子替换本地源码目录，然后继续本机匹配 fat binary 构建。已有可用源码时不会读取或下载归档。恢复失败会显示“gpu-burn 源码缺失”，不会误报为硬件压测异常。
...
任务历史外层卡（单次卡与批次子任务摘要）通过 `taskError.ts` 将已识别错误转换为中文短结论；任务详情的“失败原因”保留 `failure_reason` / `error_message` 原文，执行日志和诊断证据也保留原始文本，兼顾快速扫描与排障取证。若压测报告为 FAIL 但无更具体证据，卡片保守显示“GPU 压测报告未通过”，不会将“任务执行成功”误作压测通过。
...
前端任务展示通过 `frontend/src/utils/taskPresentation.ts` 接受单任务与批次详情的结构兼容输入，统一计算功能分类、模块名称、GPU 精度、受控套件动作、批次步骤名称和最终状态；仪表盘、服务器详情与历史任务均传入完整任务对象，`script` 只有缺少文件上下文时才回退显示为“服务器环境”，避免把 MPI 编译环境误标为基础环境。`taskError.ts` 统一选择 `outcome_message`、`failure_reason`、`error_message` 并为历史列表卡片映射已识别英文错误的中文结论；详情层不转换原始失败字段，日志与诊断证据也保留原始文本。后端仅在确认整个安装任务未产生安装动作时返回成功跳过说明：单组件或依赖步骤跳过不计入，oneAPI 与 AOCC/AOCL/OpenMPI 等组合安装必须全部目标组件均已存在。单次和批次组件只保留布局及各入口已有的兜底文案。

### 压测运行续租与报告回收

- 远端脚本先写入临时 XLSX，再原子替换最终文件名；采集端下载到本地 `.part`，完成 ZIP 完整性校验后再原子入库。
- `cpu_mem_stress_report.sh`、`gpu_stress_report.sh`、`disk_stress_report.sh` 当前均要求 `openpyxl` 成功安装并复核可导入，且 XLSX 文件非空，才以成功退出；依赖安装、模块复核或 XLSX 生成失败均明确失败，TXT/CSV-only 不得恢复为 SUCCESS。
