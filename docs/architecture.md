...
失败原因遵循“先取证，再归因”：只有命中回收日志、报告、退出码或状态机中的明确证据时，才在详情展示具体根因；例如 Bash `unbound variable` 会自动提取变量名并明确归因为脚本实现错误。报告结果为 `FAIL` 时，摘要会从已回收日志的 `Reason:` / `判定原因:` 提取失败原因。执行状态为 `SUCCESS`、报告状态为 `UNKNOWN` 且诊断已确认完成的非压测报告型任务不写失败原因，前端显示“无报告”。其余无法确认的失败场景才显示“未能从已回收日志确认具体根因，请查看任务日志与结果文件”。“疑似重启”等推测不作为失败原因写入详情，仍可在诊断建议中提示核查方向。
