...
- 磁盘压测 `v2026.08.21.5` 在 fio 单阶段能力之上，先以用户指定时长执行性能主测，再执行持久化校验。性能主测使用 4K `randrw`、读 30%/写 70%、`libaio`、`direct=1`，并按 HDD/SSD/NVMe 使用 4/16/32 队列深度；报告同时给出每 worker 队列深度与总队列深度。持久化校验使用 4K `randwrite`、`psync`、逐写 `fdatasync=1` 与 CRC32C，并取消该阶段的 `time_based/runtime`，确保 fio 在固定工作集写满后进入实际回读校验；工作集按性能主测时长分档：不超过 3 分钟为 8MiB/worker、3–60 分钟为 32MiB/worker、超过 60 分钟为 256MiB/worker。任一阶段的 fio JSON 缺失、无效、I/O 错误或校验失败均判 FAIL。持久化阶段是性能主测后的附加尾段，任务完成时间会略长于用户设定时长；TXT/XLSX 明确分区展示性能与持久化指标。
