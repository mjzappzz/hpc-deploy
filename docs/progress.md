...
### 清理中心
- 本地结果文件按 task 目录聚合查看与批量删除，自动显示元数据（服务器名称/任务类型/脚本名/日期标签）
- 本地结果按目录 mtime 降序排列（最新在前）
- 本地结果批量推断批次归属（同时包含 GPU/CPU内存/磁盘 三种类型目录自动标记"疑似批次"）
- 自动清理本地报告面板：开关/保留天数/执行时间配置、最近一次执行结果展示
- 自动清理按目录 mtime 判断是否过期，跳过运行中任务，删除前路径防逃逸校验
- Apptainer 镜像目录只读查看
- 远端单台/全部在线服务器临时目录扫描与清理（支持标签筛选）
- 远程扫描自动匹配数据库任务记录，展示关联信息（display_title、server_name、batch_id）
- 远端任务目录按 mtime 降序排列
- 远端单个任务目录使用 HMAC 签名 delete_key 防篡改删除
