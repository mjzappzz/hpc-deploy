...
- 执行自带的基础环境配置、NVIDIA 驱动、CUDA、MPI 编译环境和 GPU / CPU内存 / 磁盘压测；磁盘压测会在远端启动前识别实际底层设备，按 HDD、SSD、NVMe 或 RAID 逻辑盘选择安全的默认 worker 与工作集，并预留文件系统容量余量。多服务器时按每台服务器分别展示并默认全选其已挂载磁盘；每台只压自己勾选的目录，GPU、CPU/内存等前置阶段完成后，同服务器的已选磁盘并行运行，每盘生成独立子任务与报告；批次 ZIP 保留报告原始文件名（含挂载目录标识）。CPU/内存报告会采集 CPU Busy、I/O Wait、Steal、内存目标达成率和可用温度，并在运行中保留内存安全余量，负载未达到验证阈值或安全余量持续不足时明确判定失败。XLSX 依赖不可用时仍保留 TXT/CSV 结果。
