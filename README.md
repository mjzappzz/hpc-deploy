...
| GPU 软件部署 | Linux NVIDIA `.run` 驱动库（GeForce / Data Center）与 CUDA Toolkit 11.8、12.0–12.6、12.8、12.9、13.0 自动安装 |
...
安装脚本会安装基础依赖和 Nginx、创建后端虚拟环境、构建前端、发布静态文件、初始化运行目录，并注册和启动后端 systemd 服务。部署详情与日常更新见 [deploy/README.md](deploy/README.md)。

## 使用流程

1. 在“服务器管理”新增目标服务器并完成 SSH 探测。
2. 在“资产库管理”统一上传 Linux NVIDIA 驱动、受控脚本或待分发的 Apptainer `.sif` 镜像；脚本按基础环境配置、MPI 编译环境配置、Linux 服务器压测和 Apptainer 分类展示。Windows 压测脚本仅在“Windows 压测（独立）”页面上传、复制或下载。
3. 在“执行任务”选择基础环境配置、GPU 驱动安装、MPI 编译环境配置或 Linux 服务器压测；基础环境与 GPU 软件支持多选并按固定顺序串行执行。
