...
2. 在“资产库管理”统一上传 Linux NVIDIA 驱动、受控脚本或待分发的 Apptainer `.sif` 镜像；驱动库与脚本知识库分区展示。Windows 压测脚本仅在“Windows 压测（独立）”页面上传、复制或下载。
3. 在“执行任务”选择服务器与脚本；GPU 驱动按 GeForce / Data Center 选择具体版本，CUDA 可选择 Toolkit 版本，实时查看日志、资源指标和任务状态。
