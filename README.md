...
- **GPU 运行指标补齐** — 历史任务的单次任务与批次详情 GPU 面板现直接展示 `nvidia-smi` 返回的风扇转速、实时/上限功耗、性能状态和 PCIe Bus-ID，并保留利用率、显存与温度。
- **管理员模式与审计收口** — 顶部可切换普通/管理员模式；可选 5 / 15 / 30 / 60 分钟或“本标签页持续”。会话使用 HttpOnly Cookie 并绑定当前浏览器标签页，刷新可恢复；手动退出、超时或关闭标签页后保持普通模式。管理员模式可直接查看审计日志；普通模式点击该入口会给出权限提示。
...
- **日志、时间与结果维护收口** — 数据库日志按任务汇总并默认按最后记录时间排序；高频下载/终端刷新输出不再写入数据库或回收日志。系统设置打开时自动分页加载本机结果与数据库日志；所有展示和日志下载时间固定为北京时间。自动清理按任务结束时间（缺失时创建时间）同步清理本机结果与数据库任务日志。
...
- **运行数据管理** — 系统设置集中展示数据库、keys、脚本库、artifacts、备份、Apptainer、本机结果文件大小和路径；打开页面自动分页加载本机结果与数据库日志，支持按任务/批次展示和共用保留期自动清理
