...
const commonDisk = ['-Mode disk', '-DiskMinutes {minutes}', '-IntervalSeconds {interval}', '-DiskFileSize {diskSize}', '-DiskIoProfile both', '-DiskBothTimePolicy split', '-ReportBase "$HOME\\Downloads"', '-ToolRoot "$HOME\\Downloads\\stress_tools"', '-EnableCpuTemperature $true', '-AutoDownloadLibreHardwareMonitor $true', '-StartLibreHardwareMonitorGui $false', '-CleanupDiskSpdTempFiles $true', '-CleanupAllFixedDrives $true', '-AutoHardwareThreshold $true', '-LenientCustomerReport $true']
