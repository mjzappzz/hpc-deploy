...
- `scripts/redeploy_frontend_only.sh`
  - 前端独立热发布入口；构建到独立版本目录，校验产物和 `nginx -t` 后原子切换静态资源，不重启后端、不停止远端任务；失败时保留当前版本，可通过 `current` 符号链接切回上一版

## SSH 身份与极限压测边界

- 新服务器首次操作前，在服务器详情读取并确认 SSH host fingerprint；指纹变化会阻断公钥部署和任务下发，确认前不执行危险远端变更。
- 密码仍用于首次接入和公钥部署。密钥连接验证成功后，操作员可单独确认清除平台保存的密码；该操作不修改远端账户密码或公钥。
- 极限压测是 GPU + CPU/内存的原子任务，不包含磁盘。提交前会复查 SSH 身份、GPU、内存余量、远端/控制器空间、依赖、温度采样和资源冲突；缺少温度采样只告警，不伪造温度保护结论。
- 极限任务预检快照保存在任务参数和日志中，冲突拒绝写入审计；历史详情可查看提交前依据。测试机断网时不得绕过 SSH 身份确认或强行下发。
