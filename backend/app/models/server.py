...
class Server(Base):
    ...
    ssh_host_key_algorithm: Mapped[str | None] = mapped_column(String(80), nullable=True)
    ssh_host_key_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    key_auth_verified_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    ...
    cpu_sockets: Mapped[int | None] = mapped_column(Integer, nullable=True)
    cpu_physical_cores: Mapped[int | None] = mapped_column(Integer, nullable=True)
    cpu_logical_threads: Mapped[int | None] = mapped_column(Integer, nullable=True)
    memory_info: Mapped[str | None] = mapped_column(Text, nullable=True)
    disk_info: Mapped[str | None] = mapped_column(Text, nullable=True)
    disk_inventory: Mapped[str | None] = mapped_column(Text, nullable=True)
