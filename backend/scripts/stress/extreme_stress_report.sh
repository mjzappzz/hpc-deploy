...
if [ "$STOP_TRIGGERED" -eq 1 ]; then
  ...
  printf '{"report_status":"FAIL","gpu_exit":%s,"cpu_mem_exit":%s,"gpu_pid":%s,"cpu_mem_pid":%s,"start_skew_ms":%s,"temperature_monitor":"%s","temperature_limit_c":%s,"ssh_failure_threshold":%s,"reason":"%s"}\n' "$gpu_rc" "$cpu_rc" "$gpu_pid" "$cpu_pid" "$skew_ms" "$TEMPERATURE_MONITOR_STATUS" "$GPU_TEMP_LIMIT_C" "$SSH_FAILURE_THRESHOLD" "$STOP_REASON" > "$RESULT_FILE"
