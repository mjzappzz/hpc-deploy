...
<#
NVIDIA GeForce / RTX FurMark2 + y-cruncher + DiskSpd Stability Report v96 DiskSpd Exit-Code Reliability Fix
Windows PowerShell 5.1+
ASCII-safe script body. Chinese text in HTML is encoded as HTML entities where needed.

V96 DiskSpd/report reliability corrections:
- Preserve v95 actual module execution-state reporting.
- Wait for redirected DiskSpd output to flush before reading ExitCode.
- Treat an unavailable/blank ExitCode as diagnostic information, not a failed test.
- Only a validated non-zero ExitCode is written as ERROR.
- Ignore the legacy v95 empty-exit-code pseudo-error during report parsing.
- Valid DiskSpd output remains authoritative for performance evaluation.
#>
