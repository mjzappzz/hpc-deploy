...
class DiskStressDirectoryContractTests(unittest.TestCase):
    def test_disk_profile_is_selected_from_the_backing_device_when_workers_are_not_overridden(self) -> None:
        ...
        self.assertIn('elif [[ "$DISK_PROFILE" == hdd* ]]; then', source)
        self.assertIn('FIO_DURABILITY_PROFILE="HDD > 60 minutes (capped)"', source)
        self.assertIn('[STAGE] durability_verify_start', source)
        self.assertIn('耐久校验中：', source)
